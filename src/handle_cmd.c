...
int	handle_cmd(t_minishell *sh)
{
	...
	while (cur)
	{
		...
		{
			fprintf(stderr, "program:\tcur->content %s\n", cur->content);
			//ft_free(path);
			if (!is_builtin(cur, cur->content))
				path = find_path(sh->envp, cur->content);
			if (path != NULL || is_builtin(cur, cur->content))
				init_argv(sh, cur);
			fprintf(stderr, "program:\thello %s\n", path);
			excecute_cmd(sh, cur, prev_fds, path);	// 명령어 수행하는 데로 간다.
			ft_free(path);

			fprintf(stderr, "\n\nafterprogram:\thello %s\n", path);
